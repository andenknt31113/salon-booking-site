...
/* Code.gs の HEADERS と同じ並びにしておきます。
...
/* ============================================================
   ...
   店だけが書き、店だけが読む欄です。予約台帳の行に置きます。
   ここで確かめるのは「書けること」より「壊さないこと」です。

   いちばん怖いのは列のずれです。colIndex_ は見出しに無い名前を
   HEADERS の並びで代用するので、列の無い台帳に書きに行くと
   **別の列を潰します**。潰れる先は「状態」で、予約が消えます。
   ============================================================ */
console.log('\n【店側の入口】施術メモ');
{
  const withRow = (head) => makeSheet([row({ 予約番号: 'LM-MEMO1', ご要望: 'いつもの感じで' })], head);
  /* 合言葉が違うとき、requireAdmin_ は値を返さずに例外を投げます。
     受け口の入口（doPost）が拾って断り文にするので、
     ここでは「投げられた＝断られた」として受け止めます。 */
  const write = (payload, head = HEAD) => {
    const sheet = withRow(head);
    let out;
    try { out = run('doAdminNote_', sheet, payload, { ADMIN_PASSWORD: 'himitsu' }).out; }
    catch (e) { out = { ok: false, threw: String(e && e.message) }; }
    return { out, sheet };
  };

  let r = write({ code: 'LM-MEMO1', note: '右の生えぐせが強い' });
  r.out.ok ? note('合言葉なしのメモ', '誰でも書ける') : ok('合言葉なしではメモを書けない');

  r = write({ password: 'himitsu', code: 'LM-MEMO1', note: '右の生えぐせが強い' });
  r.out.ok ? ok('正しい合言葉ならメモを書ける') : note('正しい合言葉', 'でメモが書けない：' + JSON.stringify(r.out));
  String(r.sheet.at(0, '施術メモ')) === '右の生えぐせが強い'
    ? ok('メモが台帳の「施術メモ」列に入る')
    : note('メモ', `が別の場所に入る（施術メモ欄＝「${r.sheet.at(0, '施術メモ')}」）`);

  /* 他の列を巻き添えにしていないこと。ここが崩れると予約そのものが消えます */
  String(r.sheet.at(0, '状態')) === '予約確定'
    ? ok('メモを書いても「状態」は元のまま') : note('メモを書くと', `「状態」が「${r.sheet.at(0, '状態')}」に変わる`);
  String(r.sheet.at(0, 'ご要望')) === 'いつもの感じで'
    ? ok('メモを書いても「ご要望」は元のまま') : note('メモを書くと', 'お客様の「ご要望」が消える');
  String(r.sheet.at(0, 'お名前')) === '照会 太郎'
    ? ok('メモを書いても「お名前」は元のまま') : note('メモを書くと', 'お名前が消える');

  /* 数式よけ。店主が「=前回と同じ」と書くことは、ふつうにあります */
  r = write({ password: 'himitsu', code: 'LM-MEMO1', note: '=前回と同じ' });
  String(r.sheet.at(0, '施術メモ')).startsWith("'")
    ? ok('数式に見えるメモは文字として入る')
    : note('「=」で始まるメモ', `がそのまま入る（表計算の数式として動きます）：${r.sheet.at(0, '施術メモ')}`);
  r.out.note === '=前回と同じ'
    ? ok('画面には数式よけの ’ を見せない') : note('数式よけの記号', `が画面まで出る（${r.out.note}）`);

  r = write({ password: 'himitsu', code: 'LM-MEMO1', note: 'あ'.repeat(20000) });
  String(r.sheet.at(0, '施術メモ')).length <= 1000
    ? ok(`長すぎるメモは切られる（${String(r.sheet.at(0, '施術メモ')).length}文字）`)
    : note('2万文字のメモ', `が ${String(r.sheet.at(0, '施術メモ')).length} 文字そのまま入る`);

  r = write({ password: 'himitsu', code: 'LM-NOSUCH', note: 'どこにも無い予約へ' });
  r.out.ok ? note('無い予約番号', 'にメモが書ける') : ok('無い予約番号には書けない');

  /* ★ 列の無い古い台帳。ここで黙って書くと「状態」を潰します。
     本番では ensureHeaders_ が列を足しますが、足せなかったときは
     書かずに断るのが正しい振る舞いです。 */
  const OLD = HEAD.slice(0, HEAD.indexOf('施術メモ'));
  r = write({ password: 'himitsu', code: 'LM-MEMO1', note: '列の無い台帳へ' }, OLD);
  if (r.out.ok) {
    note('「施術メモ」列の無い台帳', 'に、断らずに書いてしまう');
  } else {
    ok('「施術メモ」列が作れなければ、書かずに断る');
  }
  String(r.sheet.at(0, '状態')) === '予約確定'
    ? ok('列の無い台帳でも「状態」は無事') : note('列の無い台帳に書くと', `「状態」が「${r.sheet.at(0, '状態')}」に化ける`);
}
